...
```{r functions}
generate_umi_count <- function(mean = umi_mean, 
                               std = umi_std) {
  # UMI 분자 수 생성
  max(as.integer(rnorm(1, mean, std)), 10)
}

binomial_test_pvalue <- function(alt_reads, 
                                 total_reads, 
                                 error_rate) {
  # 이항분포 검정
...
generate_molecule_counts <- function(n_unique_molecules, 
                                     true_vaf) {
...
generate_variant_data <- function(pos, 
                                  gene_name, 
                                  region, 
                                  hotspots_df) {
...
simulate_gene_variants <- function(gene_name, 
                                   region, 
                                   n_positions) {
  # 특정 유전자의 변이 시뮬레이션
  
  cat(sprintf("[%s] 시뮬레이션 중...\n", gene_name))
...
cat(sprintf("  - 전체 포지션: %s\n", 
            format(nrow(sim_ctdna), 
                   big.mark = ",")))
cat(sprintf("  - TP53: %s\n", 
            format(sum(sim_ctdna$gene == "TP53"), 
                   big.mark = ",")))
cat(sprintf("  - EGFR: %s\n", 
...
kable(true_vars, digits = 2, format = "html")
...
for (filter_name in c("stringent", "sensitive")) {
  ...
  performance_results[[filter_name]] <- data.frame(
    Filter = tools::toTitleCase(filter_name),
    Sensitivity = sprintf("%.2f%%", sensitivity * 100),
    FPR = sprintf("%.4f%%", fpr * 100),
    LowVAFDetection = sprintf("%d/%d", low_vaf_success, nrow(low_vaf_variants))
  )
}

performance_df <- do.call(rbind, performance_results)
rownames(performance_df) <- NULL

cat("\n성능 요약 테이블:\n")
kable(performance_df, format = "html")
performance_df %>% gt::gt()
```

# 6. 데이터 저장

```{r}
sim_ctdna %>% head(10) %>% gt::gt()
```

```{r}
stringent_variants <- sim_ctdna %>% filter(pass_stringent == TRUE)

stringent_variants %>% gt::gt()
```
...
cat(sprintf("💾 데이터 저장: ctdna_simulated_data.csv\n"))
...
이 분석은 **ctDNA 변이콜링 시뮬레이션**을 통해:

- **6,000개 포지션** 데이터셋 생성
- 3개 주요 암 유전자 (TP53, EGFR, KRAS) 시뮬레이션
- **11개의 진정 변이** 포함
- **Stringent와 Sensitive 2가지 필터** 적용 비교
- **90%+ 민감도**와 **0% 위양성률** 달성

생성된 CSV 파일들은 추가 분석이나 시각화에 사용할 수 있습니다.
